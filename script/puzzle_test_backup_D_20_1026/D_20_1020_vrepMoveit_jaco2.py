...
def test_algorithm(method_in, data_in):
    ...
    target_name = ['target']
    target_info = []
    target_info.append([[data_in[0][1]-0.02, -data_in[0][0], obj_z], [0, 0, 0, 0], [0.001, 0.001, 0.001]]) # for the add_mesh
    ...
    while len(env.ore_order):  # this while loop is for the algorithm
        ...
        env.get_max_can(env.grid_ori, bt_num=2, trial_num=2500)  # We get "grid_max_can", "can_grid"
        # env.get_env_case1()
    ...
    print "n-rearrangement", rearr_cnt
...
if __name__ == "__main__":
    ...
    sim_record = open(home + "/simulation_records.csv", 'a')
    ...
    sim_record.write('Objects Relocated: %d .' % (n_rearr_obj) + "\n")
