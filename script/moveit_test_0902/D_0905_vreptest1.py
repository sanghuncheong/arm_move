...
class vrep_env_jaco:
    ...
    def pick_and_place(self, pick_pose, pick_object_name, place_pose):
        ret_pick_pose = env.pick(env.obs_pos, pick_pose, vrep_env.get_current_joint(joint_names_jaco))
        vrep_env.close_hand(1)
        CLF.att_box_client('gripper', pick_object_name)

        env.move_to([[ret_pick_pose[0][0] + 0.2, ret_pick_pose[0][1], ret_pick_pose[0][2]], ret_pick_pose[1]])
        ...
        env.move_to([[ret_place_pose[0][0] - 0.15, ret_place_pose[0][1], ret_place_pose[0][2]], ret_place_pose[1]])
        ...
        env.move_to([[ret_place_pose[0][0] + 0.2, ret_place_pose[0][1], ret_place_pose[0][2]], ret_place_pose[1]])
        env.back_home(vrep_env.get_current_joint(joint_names_jaco))
...
if __name__ == "__main__":
    ...
    while len(env.ore_order):  # this while loop is for the algorithm
        ...
        # ret_pick_pose = env.pick(env.obs_pos, sel_obs_pos, vrep_env.get_current_joint(joint_names_jaco))
        # vrep_env.close_hand(1)
        # CLF.att_box_client('gripper', obstacle_name[env.ore_order[0]])
        #
        # env.move_to([[ret_pick_pose[0][0]+0.2, ret_pick_pose[0][1], ret_pick_pose[0][2]], ret_pick_pose[1]])
        # env.back_home(vrep_env.get_current_joint(joint_names_jaco))
        # # CLF.car(0.05)
        #
        # ret_place_pose = env.place(env.obs_pos, env.obs_pos[env.ore_order[0]], vrep_env.get_current_joint(joint_names_jaco))
        # env.move_to([[ret_place_pose[0][0] - 0.15, ret_place_pose[0][1], ret_place_pose[0][2]], ret_place_pose[1]])
        # vrep_env.open_hand(1)
        # CLF.det_box_client(obstacle_name[env.ore_order[0]], [env.object_z, -sel_can_pos[1], sel_can_pos[0]], obstacle_info[env.ore_order[0]][1], obstacle_info[env.ore_order[0]][2], 'blue')
        #
        # env.move_to([[ret_place_pose[0][0]+0.2, ret_place_pose[0][1], ret_place_pose[0][2]], ret_place_pose[1]])
        # env.back_home(vrep_env.get_current_joint(joint_names_jaco))
    ...
    if len(env.ore_order)==0:
        vrep_env.pick_and_place(env.tar_pos, 'target', [0.01, -0.046])
