...
from D_01_envClass4altest import EnvInfo as EI
from D_01_envClass4altest import CanInfo as CI
    ...
    while len(env.ore_order):  # this while loop is for the algorithm
        ...
        for i in range(len(t_sel_can_index)):
            ...
            if method == 'deep':
                ...
                t_sel_can_dist.append(np.sqrt((env.rob_pos[1] - s_v[0][t_sel_can_index[i]].pos[0]) ** 2 + (-env.rob_pos[0] - s_v[0][t_sel_can_index[i]].pos[1]) ** 2)+np.sqrt((env.tar_pos[1] - s_v[0][t_sel_can_index[i]].pos[0]) ** 2 + (-env.tar_pos[0] - s_v[0][t_sel_can_index[i]].pos[1]) ** 2))
            else:
...
if __name__ == "__main__":
    ...
    sim_record = open(home + "/simulation_records_deep_20.csv", 'a')
