def tree_making(ob, target_position, target_orientation, obstacle_position, obstacle_orientation, wall_position, Body_position, Jaco_tip_position, d_max):
    ...
    for i in range(ob):  # change!
        ...
        in_ob = len(obstacle_position) + len(wall_position)
        VFH = VP.influence(in_ob, target_position, obstacle_position + wall_position, Body_position, d_max, eta_round)
        tmp = []
        if VFH[3] != 0:
    ...
    if check_flag == 1:
        ...
        dummy = 1
    print "we need to rearrange", tree[len(tree) - 1], 'process time =', stop - start
