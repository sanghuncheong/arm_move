...
class EnvInfo:
    ...
    def get_env_case1(self):
        while 1:
            ...
            self.obs_grid = [[5, 70], [5, 50], [5, 10],
                             [20, 70], [20, 50], [15, 30], [22, 25], [20, 10]]
            self.tar_grid = [30, 30]
            ...
            if len(ore_order) > min_ore:
                ...
                tm_obs_pos.extend(self.obs_wall)
                ob = len(tm_obs_pos)
                self.ore_order = TM_noplot(ob, tm_tar_pos, tm_tar_ori, tm_obs_pos, tm_obs_ori, self.rob_pos, self.rob_pos, self.d_max)
    ...
    def get_max_can_case1(self):
        ...
        self.can_grid = [[30, 70], [30, 50], [5, 30], [30, 10]]
        self.can_pos = []
    ...
    def get_cp(self, in_can_info):
        ...
        for ci in range(len(tmp_can_info)):
            # print "\ncan ", ci, "th has A, BT :", tmp_can_info[ci].A, tmp_can_info[ci].BT
            if tmp_can_info[ci].A == 0 and tmp_can_info[ci].BT == 0:
                tmp_cf.append(tmp_can_info[ci])
                tmp_cf_index.append(ci)
        return tmp_cf, tmp_cf_index
    ...
    def get_c_ore(self, in_can_info):
        ...
        for ci in range(len(in_can_info)):
            ...
            ore_order = TM_noplot(tm_ob, tm_tar_pos, tm_tar_ori, tm_obs_pos, tm_obs_ori, self.rob_pos, self.rob_pos, self.d_max)
            ore_order.pop()
