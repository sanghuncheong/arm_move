...
if __name__ == '__main__':
    ...
    for n_obs_max in n_obs_list:
        for test_i in range(10):
            ...
            ori_can_pos = copy.deepcopy(env.can_pos)
            ori_can_grid = copy.deepcopy(env.can_grid)
            ori_obs_pos = copy.deepcopy(env.obs_pos)
            ori_obs_grid = copy.deepcopy(env.obs_grid)
            for method in compare_method:
                rearr_cnt = 0       # total rearrangement count
                rearr_add_cnt = 0   # additional rearrangement count

                env.obs_pos = copy.deepcopy(ori_obs_pos)
                ...
                while len(env.ore_order):   # this while loop is for the algorithm algorithm will go on until it can access to target
                    ...
                    for i in range(len(t_sel_can_index)):
                        ...
                        if method == 'deep':
                            t_sel_can_dist.append(np.sqrt((env.rob_pos[0] - s_v[t_sel_can_index[i]].pos[0]) ** 2 + (env.rob_pos[0] - s_v[t_sel_can_index[i]].pos[0]) ** 2))
                        else :
