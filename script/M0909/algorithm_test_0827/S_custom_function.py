...
def draw_environment(ws_zero_pos, ws_wd, rob_pos, obs_pos, can_pos, tar_pos):
    import matplotlib.pyplot as plt
    fs = 10
    # fs = 10
    new_fig = plt.figure(figsize=(fs, fs))
    ...
    workspace = plt.Rectangle(ws_zero_pos, ws_wd[0], ws_wd[1], color='k', fill=False)
    ax.add_artist(workspace)

    robot = plt.Circle(rob_pos, 0.04, color='gray')
    ax.add_artist(robot)

    for obs_i in obs_pos:
        obstacle = plt.Circle(obs_i, 0.03, color='red')
        ax.add_artist(obstacle)
        plt.text(obs_i[0], obs_i[1], 'o'+str(obs_pos.index(obs_i)), fontsize=10, ha='center')

    for can_i in can_pos:
        candidate = plt.Circle(can_i, 0.03, color='pink', fill=False, linewidth=2, linestyle=':')
        ax.add_artist(candidate)
        plt.text(can_i[0], can_i[1], 'c'+str(can_pos.index(can_i)), fontsize=10, ha='center')


    target = plt.Circle(tar_pos, 0.03, color='green')
    ax.add_artist(target)

    # plt.show()


def draw_check_pos(check_pos, check_color):
    import matplotlib.pyplot as plt

    ax = plt.gca()
    check_point = plt.Circle(check_pos, 0.03, color=check_color, alpha=0.3)
    ax.add_artist(check_point)
...
if __name__ == "__main__":
    ...
    ws_wd = [0.45, 0.9]
    rob_pos = [0, 0]
    obs_pos = [[0.75, 0.0], [0.7, 0.5]]
    # obs_r = [0.03, 0.03]
    can_pos = [[0.75, 0.2], [0.7, -0.2]]
    # can_r = [0.03, 0.03]
    tar_pos = [1.1, 0.0]
    # tar_r =  [0.03]
    # draw_environment(ws, ws_wd, rob_pos, obs_pos, can_pos, tar_pos)
