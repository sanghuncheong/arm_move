...
if __name__ == "__main__":
    ...
    d_max = 1.0
    # re_order = TM_P.tree_making(ob, target_position=tar_pos, target_orientation=[0.0, 0.0, 0.0], obstacle_position=obs_pos, obstacle_orientation=obs_ori, Body_position=rob_pos, Jaco_tip_position=rob_pos, d_max=d_max)
    re_order = [7, 3, 'T']
    ...
    for i in range(len(rearrange_order)-1):
        ...
        obs_re_r.pop(rearrange_order[i])
    ...
    for i in can_cen_grid:
        ...
        # print("vfh", vfh)
        if vfh_ret[3] == 0:
            print("candidate", i, "occludes the target", vfh_t_pos)
    ...
    for i in can_cen_grid:
        for ore_posi in ore_pos:
            ...
            # print("vfh", vfh_ret)
            if vfh_ret[3] == 0:
                print("candidate", i, "occludes the obstacle to remove", ore_posi)
                print("candidate penalty")
                if (i in can_pan) == 0:
                    can_pan.append(i)
    for i in can_pan:
    ...
    for i in can_cen_grid:
        ...
        # print("vfh", vfh_ret)
        if vfh_ret[3] == 0:
            print("candidate", i, "can not be rearranged")
            print("candidate penalty")
            if (i in can_pan) == 0:
                can_pan.append(i)
    for i in can_pan:
    ...
    print("circle before penalty", can_cen_grid)
    for i in can_pan:
        can_cen_grid.remove(i)
    print("circle after penalty", can_cen_grid)


    grid_val_can_rviz1 = copy.deepcopy(grid_del)
    for i in can_cen_grid:
        xi, yi = i
        grid_val_can_rviz1 = obstacle_circle(grid_val_can_rviz1, [xi, yi, 0.03], 3)  # target

    print "\nbefore checking OCC, check if the candidate occlude the target"
    can_pan = []
    for ci in can_cen_grid:
    ...
    grid_val_can_rviz2 = copy.deepcopy(grid_del)
    for i in can_cen_grid:
        xi, yi = i
        grid_val_can_rviz2 = obstacle_circle(grid_val_can_rviz2, [xi, yi, 0.03], 3)  # target

    '''
    print("\ncheck with rviz if the candidate occlude the obstacle to remove")
    can_pan = []
    # obstacle_re = [[50, 10]]
    for i in can_cen_grid:
        for ore_posi in ore_pos:
            xi, yi = ws_zero_pos[0] + ore_posi[0] * GRID_SIZE, ws_zero_pos[1] + ore_posi[1] * GRID_SIZE
            add_box_client('can_check', [obstacle_info[0][0][0], -yi, xi], [-0.707, 0.0, -0.707, 0.0], [0.06, 0.06, 0.2], 'red')
    ...
    if len(can_cen_grid) >= len(ore_grid):
        ...
        for ci in can_cen_grid:
            ...
            xi, yi = ws_zero_pos[0] + ci[0] * GRID_SIZE, ws_zero_pos[1] + ci[1] * GRID_SIZE
            f_check_pos = [obstacle_info[0][0][0], -yi, xi - 0.15]
            f_check_ori = [-0.00145713772037, -0.998970756926, 0.0364956710831,  0.0268955302573]
            fes_ret = feasible_check_client('arm', f_check_pos, f_check_ori)
            if fes_ret == 0:
                print("candidate", ci, "can not be rearranged")
    ...
    draw_grid_info(grid_val_can_rviz1)  # grid map after considering acc-possibility with rviz
    draw_grid_info(grid_val_can_rviz2)  # grid map after considering acc-possibility with rviz
    plt.show()
