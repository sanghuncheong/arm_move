...
if __name__ == '__main__':
    ...
    while 1:
        ...
        for i in range(len(env.ore_order)):
            ...
            for ore_i in range(i + 1):
                in_obs_pos.remove(env.obs_pos[env.ore_order[ore_i]])
            ret_pos, ret_index = env.get_cf(in_can_info)
            t_cf_pos.append(ret_pos)
            t_cf_index.append(ret_index)
            print "\n step", i, " has # of cf pos:", len(t_cf_pos[i]), "index", t_cf_index[i]
        ...
        if draw_figs == 1:
            ...
            for step_i in range(1):
                ...
                for cf_i in range(len(t_b[step_i])):
                    xi = (t_cf_pos[step_i][cf_i][0]-env.ws_zero[0])*G2P_SIZE
                    yi = (t_cf_pos[step_i][cf_i][1]-env.ws_zero[1])*G2P_SIZE
        ...
        for i in range(len(t_sel_can_index)):
            t_sel_can_dist.append(np.sqrt((env.tar_pos[0]-t_cf_pos[0][t_cf_index[0][i]][0])**2 + (env.tar_pos[1]-t_cf_pos[0][t_cf_index[0][i]][0])**2))
