...
import S_custom_function as CF
...
if __name__ == "__main__":
    ...
    obs_wall.extend(CF.linspace2D(ws_side[0], ws_side[1], round(ws_w*GRID_SIZE/OBJ_RADIUS)))
    print("obstacles for wall", len(obs_wall), type(obs_wall), obs_wall)
    obs_wall.extend(CF.linspace2D(ws_side[1], ws_side[2], round(ws_d*GRID_SIZE/OBJ_RADIUS)))
    print("obstacles for wall", len(obs_wall), type(obs_wall), obs_wall)
    obs_wall.extend(CF.linspace2D(ws_side[2], ws_side[3], round(ws_w*GRID_SIZE/OBJ_RADIUS)))
    print("obstacles for wall", len(obs_wall), type(obs_wall), obs_wall)
