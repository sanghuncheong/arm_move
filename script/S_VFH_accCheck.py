...
def get_pos(num_grid, num_width, grid_width, grid_depth):
    x = ((num_grid - 1) % num_width)*grid_width + grid_width/2.0
    y = ((num_grid - 1) // num_width)*grid_depth + grid_depth/2.0
    return [x, y]


def influence(ob, target, obstacle, Body_position, d_max):
    ...
    # target = get_pos(target, num_width, grid_width, grid_depth)
    '''
    plt.figure()
    for i in range(len(obstacle)):
        plt.scatter(obstacle[i][0], obstacle[i][1], c='red')
    plt.scatter(target[0], target[1], c='blue')
    plt.scatter(Body_position[0], Body_position[1], c='green')
    '''

    ob_radius = []
    for i in range(ob):
        ob_radius.append(0.035)
    target_radius = 0.035
    # for i in range(ob):
    #     obstacle.append(get_pos(obstacle_num[i], num_width, grid_width, grid_depth))
