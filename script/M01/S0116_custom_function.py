...
def get_obs_wall(ws_cen, ws_size, OBJ_R, GRID_SIZE):
    ws_w, ws_d = ws_size[0]*100, ws_size[1]*100
    ws_side = []
    ws_side.append(
    ...
    obs_wall = []
    obs_wall.extend(linspace2D(ws_side[0], ws_side[1], round(ws_w * GRID_SIZE / OBJ_R)))
    obs_wall.extend(linspace2D(ws_side[1], ws_side[2], round(ws_d * GRID_SIZE / OBJ_R)))
    obs_wall.extend(linspace2D(ws_side[2], ws_side[3], round(ws_w * GRID_SIZE / OBJ_R)))
    return obs_wall
...
def get_can_BT(in_can_info, in_obs_pos, in_tar_pos, obs_wall):
    ...
    for ci in range(len(tmp_can_info)):
        ...
        if vfh[3] == 0:
            ...
            #print ci, "BT = 0 (vfh)"
            tmp_can_info[ci].BT = 0  # BT == 0 : The candidate does not block the target.
            bt_list.append(0)
...
def get_b(in_can_info, in_obs_pos, obs_wall):
    ...
    for cb in range(len(tmp_cf)):
        ...
        for ci in range(len(tmp_cf)):  # ci: Other candidates for checking the b value
            if cb != ci:
                print "check", cf_index[ci], "' b if rearrange obstacle at", cf_index[cb]
