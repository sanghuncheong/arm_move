...
class EnvInfo:
    ...
    def get_env(self, obs_r, tar_r, min_ore):
        while 1:
            ...
            ore_order = TM_noplot(ob, tm_tar_pos, tm_tar_ori, tm_obs_pos, tm_obs_ori, self.rob_pos, self.rob_pos, d_max)
            ...
            if len(ore_order) > min_ore:
                ...
                tm_obs_pos.extend(self.obs_wall)
                ob = len(tm_obs_pos)
                self.ore_order = TM_plot(ob, tm_tar_pos, tm_tar_ori, tm_obs_pos, tm_obs_ori, self.rob_pos, self.rob_pos, d_max)
                ...
                tm_obs_pos.extend(self.obs_wall)
                ob = len(tm_obs_pos)
                tmp_order = TM_noplot(ob, tm_tar_pos, tm_tar_ori, tm_obs_pos, tm_obs_ori, self.rob_pos, self.rob_pos, d_max)
                if len(tmp_order) == 1:
                    break
        '''
